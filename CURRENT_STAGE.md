...
**Последнее обновление:** 2026-02-26 16:00  
**Обновлено после:** Resizable-столбцы в data.php, исправление ext-curl
...
- ⚠️ **Нет retry-логики** при отправке в 1С — если сервер недоступен, заказ не отправляется повторно автоматически (но ручная переотправка через кнопку 🔄 работает)
...
|------|----------|-----------------|--------|
| 2026-02-26 | Исправлен баг resend в api.php | `api.php` | Dead code `if ($action === 'resend')` перенесён в `case 'resend':` внутри switch |
...
5. **`resend` в `api.php` — исправлен** (2026-02-26). Был dead code между `break;` и `default:`. Перенесён в `case 'resend':`. Кнопка 🔄 в `data.php` теперь работает.
...
14. **Нет автоматического retry.** Если API 1С недоступен — JSON сохраняется, но не отправляется. Ручная переотправка через кнопку 🔄 в `data.php` работает.
