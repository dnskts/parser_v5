...
**Последнее обновление:** 2026-02-26 14:00  
**Обновлено после:** Финализация — пункты 13–14 в секции AI-контекста, исправлен context-keeper.md, создан .cursorrules
...
**Статистика проекта:**
- 12 PHP-файлов, ~2 900 строк PHP-кода
- 2 фронтенд-файла (JS + CSS), ~920 строк
- Итого: ~3 800 строк кода
...
> **⚠️ ВНИМАНИЕ: Функция сломана — dead code в api.php (см. секцию 9 «Известные проблемы»)**

Задуманная логика (нерабочая):
- Кнопка 🔄 рядом с каждой строкой в `data.php` → `fetch('api.php?action=resend', {file: 'имя.json'})`
- `api.php` должен читать файл из `json/`, создавать `ApiSender`, вызывать `send()`
- **Фактически:** запрос всегда возвращает HTTP 400, т.к. resend-код расположен вне switch-case
...
| `resend` | POST | Повторная отправка JSON в 1С (**⚠️ СЛОМАНО — dead code, см. секцию 9**) |

### 8.5. Навигация

Все три страницы (index, data, api_logs) имеют общую шапку с навигацией:
...
- ✅ **Ядро системы** — `Processor`, `ParserManager`, `Logger`, `ParserInterface` → `core/`
...
- ✅ **Веб-интерфейс** — панель управления, таблица заказов (49 колонок), логи API
- ❌ **Повторная отправка** — UI реализован (кнопка 🔄 в `data.php`), но бэкенд сломан (dead code в `api.php`, см. Известные проблемы)
...
- 📋 Рефакторинг: вынести `generateUUID()` из парсеров в общую утилиту
- 📋 **ПРИОРИТЕТ:** Исправить баг resend в `api.php` — перенести `if` блок (строки 186–221) в `case 'resend':` внутри switch
- 📋 Добавить поддержку новых поставщиков по мере необходимости

### Известные проблемы (⚠️)

- ⚠️ **`generateUUID()` дублируется** в `MoyAgentParser` и `DemoHotelParser` — одинаковый код в двух местах
- ⚠️ **КРИТИЧНО: Функция resend (переотправка JSON в 1С) полностью нерабочая** — dead code в `api.php`. Блок `if ($action === 'resend')` (строки 186–221) расположен между `break;` последнего `case 'clear_logs'` (строка 184) и `default:` (строка 225) внутри `switch ($action)`. В PHP при отсутствии совпадения с case выполнение переходит сразу к `default:` — код между метками достижим только при fall-through из предыдущего case, но `case 'clear_logs'` завершается `break;`. Поэтому блок resend **никогда не выполняется**. Кнопка 🔄 на странице `data.php` отправляет запрос `api.php?action=resend`, но всегда получает HTTP 400 «Неизвестное действие: resend». **Следствие:** если API 1С был недоступен в момент парсинга, JSON-файлы сохраняются, но не могут быть переотправлены через UI. **Требуется:** перенести resend-логику в `case 'resend':` внутри switch.
- ⚠️ **Следствие бага resend + отсутствия retry = потеря данных.** 
Сценарий: API 1С недоступен → XML обработан, JSON сохранён в json/, 
XML перемещён в Processed/ → отправка пропущена → пользователь 
нажимает 🔄 в data.php → получает ошибку 400 → JSON "застревает" 
навсегда. Единственный workaround: ручная отправка через curl/Postman 
или исправление бага resend.
- ⚠️ **Нет retry-логики** при отправке в 1С — если сервер недоступен, заказ не отправляется повторно автоматически (а ручная переотправка через UI также сломана — см. выше)
...
4. **UUID:** версия 4 (RFC 4122), генерируется в каждом парсере
...
4. **`generateUUID()` дублируется** в `MoyAgentParser.php` (строки 810–826) и `DemoHotelParser.php` (строки 191–200) — идентичный код. При рефакторинге вынести в `core/Utils.php` или добавить как static-метод в базовый класс.

5. **⚠️ БАГ: `resend` в `api.php` — dead code** (строки 186–221). Блок `if ($action === 'resend')` расположен между `break;` (строка 184, конец `case 'clear_logs'`) и `default:` (строка 225). В PHP switch-case код между метками достижим только при fall-through. Поскольку `case 'clear_logs'` завершается `break;`, код **никогда не выполняется** — `action=resend` всегда попадает в `default:` и возвращает HTTP 400. **Это не quirk, а баг.** Кнопка 🔄 в `data.php` → `resendJson()` → `api.php?action=resend` → всегда ошибка. **Исправление:** перенести в `case 'resend':` внутри switch.
...
14. **Следствие бага resend + отсутствие retry = риск потери данных.** Если API 1С был недоступен в момент парсинга: XML обработан, JSON сохранён в `json/`, XML перемещён в `Processed/` — но JSON **не был отправлен** в 1С. Повторная отправка через UI (кнопка 🔄 в `data.php`) **не работает** (баг, см. пункт 5). Автоматического retry нет. **Workaround до исправления бага:** вручную отправить JSON через cURL:
    ```bash
    curl -X POST -u "api_user:api_password" \
      -H "Content-Type: application/json; charset=utf-8" \
      -d @json/имя_файла.json \
      "http://10.30.88.24/TRADE/hs/CRM_Exchange/Order"
    ```
    Или: исправить баг в `api.php` (перенести строки 186–221 в `case 'resend':`) и использовать UI.
