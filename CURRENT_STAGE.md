...
**Обновлено после:** MoyAgentParser V5 (конъюнкции + маппинг полей), data.php (даты + агенты)
...
**Статистика проекта:**
- 14 PHP-файлов, ~3 600 строк PHP-кода
- 2 фронтенд-файла (JS + CSS), ~920 строк
- Итого: ~4 520 строк кода
...
- **Расширения PHP:** `ext-simplexml`, `ext-curl`, `ext-json`, `ext-mbstring`
...
## 3. Структура проекта
parser_v5/
├── config/settings.json      — интервал, last_run, api (url/login/password/timeout/enabled)
├── core/
...
├── logs/                     — app.log + api_send.log (JSON Lines)
├── tests/fixtures/           — 4 XML-фикстуры для MoyAgentParser
...
[cron/CLI] php process.php    [Web UI] index.php → кнопка «Запуск»
│                              │
▼                              ▼
...
4. **Два раздельных лога:** `app.log` (текстовый) и `api_send.log` (JSON Lines)
...
Цикл по папкам → glob(“input/$folder/*.xml”) → цикл по XML: try: result = $parser->parse(xmlFile) saveJson($result, $folder, $xmlFile) moveFile($xmlFile, “Processed/”) if ($apiAvailable): ApiSender->send(…) catch: moveFile($xmlFile, “Error/”) Logger->error(…)
...
8. Web UI
8.1. index.php — Панель управления
...
✅ Автотесты (test.php, 5 фикстур)
✅ **Тесты обновлены под V5** — 5 фикстур, ~109 assertions, покрыты конъюнкции, маппинг полей, агенты, даты, комиссии
...
config/settings.json — модифицируется автоматически (last_run), не коммитить
API пароли хранятся в settings.json открытым текстом
SSL верификация отключена в ApiSender (внутренняя сеть)
ext-curl обязателен — без него Fatal Error
...
ext-curl обязателен
settings.json модифицируется автоматически (last_run)
