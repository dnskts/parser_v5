...
description: Keeps docs/structure.md in sync with the project. Use when creating, renaming, or deleting files, adding or removing directories, or changing dependencies (composer.json, package.json, requirements.txt, etc.). Also use when the user asks to update or review the project structure.
---
...
**После внесения изменений в проект** (по правилам проекта): обновлять `docs/CURRENT_STAGE.md`, `docs/CHANGELOG_AI.md` и при необходимости `docs/structure.md`. Файл `docs/SisPrompt.md` (системный промпт для веб-нейросетей) обновлять, если изменились ключевые правила или структура (например, число фикстур, новые эндпоинты API).

**После изменения** `docs/SisPrompt.md`, `docs/CURRENT_STAGE.md`, `docs/CHANGELOG_AI.md` или `docs/structure.md`: выполнить в корне проекта `php scripts/sync-docs-to-txt.php`, чтобы обновить одноимённые файлы `docs/*.txt` (UTF-8 копии содержимого `.md`).

## Where to write

Update the file `docs/structure.md` in the project.
If `docs/structure.md` does not exist yet, create it using the template below.
...
- [ ] If any of `docs/SisPrompt.md`, `docs/CURRENT_STAGE.md`, `docs/CHANGELOG_AI.md`, `docs/structure.md` were edited, `php scripts/sync-docs-to-txt.php` was run and `docs/*.txt` mirrors are current
- [ ] Every new / renamed / deleted file is reflected in `docs/structure.md`
