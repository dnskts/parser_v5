...
Pipeline (единый)
process.php и кнопка «Запустить»: runSftpSync() → Processor.run(). Ответ run: sftp_status (пропущено/ошибки: N/скачано: N), sftp_skipped. SFTP встроен в pipeline — при каждом запуске сначала загрузка XML с SFTP в input/moyagent/, затем обработка. sftp_sync.php — только для standalone.
