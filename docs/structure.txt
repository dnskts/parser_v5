...
| Файл | Описание |
|------|----------|
| `references/import/` | Выгрузки из 1С «как есть» (JSON внутри .txt). Кладутся вручную раз в месяц, содержимое в .gitignore (кроме .gitkeep). Читаются ReferenceImporter по кнопке «Загрузить справочники»: Валюты.txt, АвиаКомпании.txt, АэропортыСтанцииЖД.txt, Агенты.txt / Пользователи.txt, Города.txt, Страны.txt, Контрагенты.txt (~24 МБ, читается потоково). |
