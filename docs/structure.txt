...
| `core/ApiSender.php` | Отправка заказов в API 1С по HTTP. Конструктор принимает конфиг api и путь к logs/api_send.log. Методы: isAvailable() — проверка доступности (HEAD, таймаут 2–3 с), send($orderData, $jsonFileName, $sourceXml) — prepareForApi() (удаление SOURCE_FILE/PARSED_AT, сворачивание CLIENT/SUPPLIER/CARRIER/CURRENCY/аэропортов в плоский UID, снятие AIRLINE/SERVICE_CLASS и UID у агентов), POST JSON, Basic Auth, запись в лог (JSON Lines), getLogEntries($limit), clearLog(). При ошибках возвращает понятные сообщения (сеть, 401, 404, 500 и т.д.). |
...
| `core/ReferenceManager.php` | Менеджер справочников. Загрузка JSON (loadReference), поиск по коду с учётом aliases (findByCode), поиск агента по ФИО (findAgentByName — точное совпадение, затем вхождение всех слов заказа в ФИО из 1С), сборка объекта агента без UID (buildAgentObject), UID клиента «РС ТЛС ООО» для ORDER.CLIENT (resolveClientUid, константа CLIENT_CODE='rstls'), обогащение ORDER полями UID (enrich — CLIENT, SUPPLIER, CARRIER, AGENT/BOOKING_AGENT, CURRENCY, аэропорты, авиакомпании, классы), синхронизация через API 1С (syncReference/syncAll), список типов (getAvailableTypes). Кеширование в памяти. При ненайденном UID: UID="", NAME=CODE, WARNING в лог. |
...
| `references/suppliers.json` | Справочник поставщиков: `[{uid, code, name, code_1c, code_mom, inn}]`. code = folder парсера (moyagent, smarttravel), name = наименование контрагента из 1С. Собирается из Контрагенты.txt по белому списку наименований; ручные записи сохраняются при повторном импорте, пустой uid дозаполняется по name. Первый элемент — запись-подсказка `_hint`/`_source`/`_template` без поля code (справочником игнорируется). |
| `references/clients.json` | Справочник клиентов: `[{uid, code, name, code_1c, code_mom, inn}]`. Содержит «РС ТЛС ООО» с code=`rstls` — его UID подставляется в ORDER.CLIENT для всех заказов. Собирается из Контрагенты.txt, формат и подсказка как у suppliers.json. |
